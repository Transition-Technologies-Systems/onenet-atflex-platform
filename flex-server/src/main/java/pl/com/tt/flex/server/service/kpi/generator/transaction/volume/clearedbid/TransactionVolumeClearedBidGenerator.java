...
/**
 * KPI - Wolumen transakcji ofert rozliczonych/Volume of transactions - cleared bids
 */
@Component
@AllArgsConstructor
public class TransactionVolumeClearedBidGenerator extends KpiGenerator {
    ...
    protected void fillSheet(XSSFWorkbook workbook, KpiDTO kpiDTO) {
        TransactionVolumeData transactionVolumeData = transactionVolumeDataFactory.createForEnergyOffers(kpiDTO.getDateFrom(), kpiDTO.getDateTo());
    ...
    private void fillProductAndDate(TransactionVolumeData transactionVolumeData, XSSFSheet sheet) {
        AtomicInteger rowIndex = new AtomicInteger(START_ROW_INDEX);
        transactionVolumeData.getNumberOfVolumesGroupingByProductNameAndDeliveryDate().forEach((key, value) -> {
            XSSFRow row = CellUtils.getRow(sheet, rowIndex.getAndIncrement());
    ...
    protected String getTemplate() {
        return "templates/xlsx/kpi/transaction/volumes/clearedbids/VolumeOfTransactions-clearedBids.xlsx";
    }
    ...
    protected String getFilename(KpiDTO kpiDTO) {
        ...
        return String.format("Volume_of_transactions-cleared_bids_%s-%s.xlsx", from, to);
    }

    @Override
    public boolean isSupported(KpiType kpiType) {
        return kpiType.equals(KpiType.VOLUME_OF_TRANSACTIONS_CLEARED_BIDS);
    }
}
