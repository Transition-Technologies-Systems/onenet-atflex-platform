...
/**
 * KPI - Wolumen ofert na energię bilansującą przesłaną do RB/Volume of balancing energy offers transferred to BM
 */
@Component
@AllArgsConstructor
public class VolumeBalancingEnergyOffersToBmGenerator extends KpiGenerator {

    private static final int START_ROW_INDEX = 1;
    private static final int DATE_OF_DELIVERY_COL_INDEX = 0;
    private static final int VOLUME = 1;

    private final VolumeBalancingEnergyOffersToBmDataFactory volumeBalancingEnergyOffersToBmDataFactory;

    @Override
    protected void fillSheet(XSSFWorkbook workbook, KpiDTO kpiDTO) {
        VolumeBalancingEnergyOffersData volumeBalancingEnergyOffersData = volumeBalancingEnergyOffersToBmDataFactory
    ...
    protected String getTemplate() {
        return "templates/xlsx/kpi/volume/balancing/energy/VolumeBalancingEnergyOffersTransferredToBM.xlsx";
    }
    ...
    protected String getFilename(KpiDTO kpiDTO) {
        ...
        return String.format("Volume_of_balancing_energy_offers_transferred_to_BM_%s-%s.xlsx", from, to);
    }

    @Override
    public boolean isSupported(KpiType kpiType) {
        return kpiType.equals(KpiType.VOLUME_OF_BALANCING_ENERGY_OFFERS_TRANSFERRED_TO_BM);
    }
}
