...
import static pl.com.tt.flex.model.security.permission.Authority.FLEX_USER_CHAT_MANAGE;
import static pl.com.tt.flex.model.security.permission.Authority.FLEX_USER_CHAT_MESSAGE_CREATE;
import static pl.com.tt.flex.model.security.permission.Authority.FLEX_USER_CHAT_VIEW;
...
public class ChatResourceUser extends ChatResource {

    public ChatResourceUser(ChatService chatService, ChatValidator chatValidator, ChatMessageService chatMessageService) {
        super(chatService, chatValidator, chatMessageService);
    }

    @GetMapping
    @PreAuthorize("hasAuthority(\"" + FLEX_USER_CHAT_VIEW + "\")")
    public ResponseEntity<List<ChatDTO>> getAllMin() {
        log.debug("{} - REST request to get all ChatMinDTO for current user", FLEX_USER_APP_NAME);
    ...
    public ResponseEntity<List<ChatRecipientDTO>> getAllRecipientsDictionary() {
        log.debug("{} - REST request to get recipients dictionary", FLEX_USER_APP_NAME);
        return super.getAllRecipientsDictionary();
    }

    @PostMapping
    @PreAuthorize("hasAuthority(\"" + FLEX_USER_CHAT_MANAGE + "\")")
    @ResponseStatus(HttpStatus.CREATED)
    public void createNewChat(@RequestBody ChatDTO chatDTO) {
        log.debug("{} - REST request to save chat: {}", FLEX_USER_APP_NAME, chatDTO);
    ...
    public ResponseEntity<List<ChatMessageDTO>> getAllMessagesForChat(@PathVariable("id") Long chatId) {
        log.debug("{} - REST request to get all messages for chat id: {}", FLEX_USER_APP_NAME, chatId);
    ...
    public ResponseEntity<FileDTO> downloadAttachedFile(@PathVariable("id") Long messageId) throws ObjectValidationException {
        log.debug("{} - REST request to download file attachment from message id: {}", FLEX_USER_APP_NAME, messageId);
        return super.getFileFromMessage(messageId);
    }

    @PostMapping("/message")
    @PreAuthorize("hasAuthority(\"" + FLEX_USER_CHAT_MESSAGE_CREATE + "\")")
    @ResponseStatus(HttpStatus.CREATED)
    public void createMessage(@NotNull @RequestPart("chatId") String chatIdString, @RequestPart(value = "content", required = false) String content,
        ...
        log.debug("{} - REST request to create a message in chat id: {}", FLEX_USER_APP_NAME, chatIdString);
    ...
    public ResponseEntity<Long> getNumberOfUnreadMessages() {
        log.debug("{} - REST request to get the number of unread messages for current user", FLEX_USER_APP_NAME);
    ...
    public void markAllInChatAsRead(@PathVariable("id") Long chatId) {
        log.debug("{} - REST request to mark all messages in chat id: {} as read", FLEX_USER_APP_NAME, chatId);
