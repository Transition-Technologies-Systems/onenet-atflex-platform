package pl.com.tt.flex.server.service.kpi.generator.volume.balancing.capacity.reserves.down.transferredtobm;
...
class VolumeBalancingOffersDownReservesToBmGeneratorTest {
    private final VolumeBalancingOffersDownReservesToBmDataFactory volumeBalancingOffersDownReservesToBmDataFactory;
    private final VolumeBalancingOffersDownReservesToBmGenerator volumeBalancingOffersDownReservesToBmGenerator;

    VolumeBalancingOffersDownReservesToBmGeneratorTest() {
        volumeBalancingOffersDownReservesToBmDataFactory = Mockito.mock(VolumeBalancingOffersDownReservesToBmDataFactory.class);
        volumeBalancingOffersDownReservesToBmGenerator = new VolumeBalancingOffersDownReservesToBmGenerator(volumeBalancingOffersDownReservesToBmDataFactory);
    }
    ...
    void givenSimpleData_expectedGenerateFile() throws KpiGenerateException, IOException {
        ...
        Mockito.doReturn(volumeBalancingOffersReservesData)
               .when(volumeBalancingOffersDownReservesToBmDataFactory)
               .create(Mockito.any(), Mockito.any());

        //given
        KpiDTO kpiDTO = KpiDTO.builder()
                              .type(KpiType.VOLUME_OF_BALANCING_SERVICE_OFFERS_DOWN_RESERVES_TRANSFERRED_TO_BM)
        ...
        //when
        FileDTO generate = volumeBalancingOffersDownReservesToBmGenerator.generate(kpiDTO);

        //then
        XSSFWorkbook expectedWorkbook = getExpectedWorkbook(
            new ClassPathResource("/templates/kpi/volume/balancing/capacity/reserves/down/VolumeBalancingOffersDownReservesTransferredToBM.xlsx")
        );
