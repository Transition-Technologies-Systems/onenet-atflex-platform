...
/**
 * Integration tests for {@link AuctionsSeriesService}.
 */
@SpringBootTest(classes = FlexserverApp.class)
@Transactional
public class AuctionSeriesServiceIT {
    ...
    @Test
    @Transactional
    public void testCreatedAuctionsDayAheadFromSeries() {
        AuctionsSeriesDTO save = auctionsSeriesService.save(auctionsSeriesDTO);
        Duration duration = Duration.between(AUCTION_SERIES_FIRST_AUCTION_DAY, AUCTION_SERIES_LAST_AUCTION_DAY);
        long totalDayOfAuctions = duration.toDays() + 1;
        List<AuctionDayAheadDTO> all = auctionDayAheadService.findAll();
        assertThat(all.size()).isEqualTo(totalDayOfAuctions);
    }
}
