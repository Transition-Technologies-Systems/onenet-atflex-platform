...
public class UserJWTControllerIT {
    ...
    public void testAuthorize() throws Exception {
        ...
        mockMvc.perform(post("/api/authenticate")
            ...
            .header("Gateway", Constants.FLEX_ADMIN_APP_NAME)
    ...
    public void testAuthorizeWithRememberMe() throws Exception {
        ...
        user.setActivated(true);
        user.setPassword(passwordEncoder.encode("test"));
        user.setRoles(Collections.singleton(Role.ROLE_FLEX_SERVICE_PROVIDER));
        user.setCreationSource(CreationSource.SYSTEM);
        ...
        mockMvc.perform(post("/api/authenticate")
            .contentType(MediaType.APPLICATION_JSON)
            .header("Gateway", Constants.FLEX_USER_APP_NAME)
            .content(TestUtil.convertObjectToJsonBytes(login)))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.id_token").isString())
            .andExpect(jsonPath("$.id_token").isNotEmpty())
    ...
    public void testAuthorizeFails() throws Exception {
        ...
        String expectedErrorMsgKey = "error.userLoginNotFound";

        LoginVM login = new LoginVM();
        login.setUsername(username);
        login.setPassword("wrong password");

        UsernameNotFoundException cause = (UsernameNotFoundException) Assertions.assertThrows(NestedServletException.class, () -> {
            mockMvc.perform(post("/api/authenticate")
                .contentType(MediaType.APPLICATION_JSON)
                .header("Gateway", Constants.FLEX_ADMIN_APP_NAME)
                .content(TestUtil.convertObjectToJsonBytes(login)));
        ...
        assertEquals(expectedErrorMsgKey, cause.getMsgKey());
        assertEquals(Status.UNAUTHORIZED, cause.getStatus());
        assertEquals("User " + username + " was not found in the database", cause.getTitle());
    }
}
