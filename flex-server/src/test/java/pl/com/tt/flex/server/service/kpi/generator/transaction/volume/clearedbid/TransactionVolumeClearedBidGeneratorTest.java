package pl.com.tt.flex.server.service.kpi.generator.transaction.volume.clearedbid;
...
import pl.com.tt.flex.server.service.kpi.generator.transaction.volume.TransactionVolumeData;
import pl.com.tt.flex.server.service.kpi.generator.transaction.volume.TransactionVolumeDataFactory;
...
class TransactionVolumeClearedBidGeneratorTest {
    private final TransactionVolumeDataFactory transactionVolumeDataFactory;
    private final TransactionVolumeClearedBidGenerator transactionVolumeClearedBidGenerator;

    TransactionVolumeClearedBidGeneratorTest() {
        transactionVolumeDataFactory = Mockito.mock(TransactionVolumeDataFactory.class);
        transactionVolumeClearedBidGenerator = new TransactionVolumeClearedBidGenerator(transactionVolumeDataFactory);
    }

    @Test
    void givenSimpleData_expectedGenerateFile() throws KpiGenerateException, IOException {
        //before
        TransactionVolumeData transactionVolumeData = getTransactionVolumeData();
        Mockito.doReturn(transactionVolumeData).when(transactionVolumeDataFactory).createForEnergyOffers(Mockito.any(), Mockito.any());

        //given
        KpiDTO kpiDTO = KpiDTO.builder()
                              .type(KpiType.VOLUME_OF_TRANSACTIONS)
        ...
        //when
        FileDTO generate = transactionVolumeClearedBidGenerator.generate(kpiDTO);

        //then
        XSSFWorkbook expectedWorkbook = getExpectedWorkbook(
            new ClassPathResource("/templates/kpi/transaction/volumes/clearedbid/VolumeOfTransactions_ClearedBids_03_10_2022-02_11_2022.xlsx"));
        verifyThatTwoWorkbookAreSame(expectedWorkbook, new XSSFWorkbook(new ByteArrayInputStream(generate.getBytesData())));
    }

    @NotNull
    private TransactionVolumeData getTransactionVolumeData() {
        ...
        return new TransactionVolumeData(numberOfVolumesGroupingByProductNameAndDeliveryDate, numberOfVolumesGroupingByProductName);
    }
