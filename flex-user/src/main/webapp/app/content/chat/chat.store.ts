...
import { EMPTY, Observable, Subscription, firstValueFrom, lastValueFrom, takeUntil } from 'rxjs';
...
export class ChatStore extends ComponentStore<ChatState> implements OnDestroy {
  ...
  readonly getUser = async () => {
    return await firstValueFrom(this.authStore.pipe(select(getUserData)));
  };
