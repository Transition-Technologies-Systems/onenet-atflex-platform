...
import { offerUpdateReminder } from '@app/core/configuration/reducers';
...
export class LoggedMainPageComponent implements OnInit, OnDestroy {
  ...
  offerUpdateReminder$ = this.store.select(offerUpdateReminder);
  ...
  ngOnInit(): void {
    ...
    this.authService.hasAuthority('FLEX_USER_VIEW_NOTIFICATION').then((hasAuthority: boolean) => {
      if (hasAuthority) {
  ...
  logout(): void {
    this.http
      .put(`${env.SERVER_API_URL}api/users/logout-current`, null)
      .pipe(
        finalize(() => {
