...
export class DayAheadComponent extends TableExtends implements AfterViewInit {
  ...
  constructor(
    ...
    this.selectedTab = this.route.snapshot.data['activeTab'] || 'capacity-auctions';
  ...
  addBid(row: AuctionDayAheadDTO): void {
    this.service
      ...
      .subscribe(resp => {
        this.modalService
          .open(AuctionBidModalComponent, { data: { model: row, type: AuctionType.DAY_AHEAD }, styleClass: 'full-view' })
          .onClose.subscribe(() => {
            this.getCollection();
          });
      });
