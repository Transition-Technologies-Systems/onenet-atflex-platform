...
import { AuctionOffersService } from '.';
...
export class AuctionOffersComponent extends TableExtends implements OnInit, OnChanges, AfterViewInit {
  ...
  get authoritiesDelete(): string {
    switch (this.auctionType) {
      case AuctionType.CMVC:
        return 'FLEX_USER_AUCTIONS_CMVC_OFFER_DELETE';
      case AuctionType.DAY_AHEAD:
        return 'FLEX_USER_AUCTIONS_DAY_AHEAD_OFFER_DELETE';
    }
  }
  ...
  get authoritiesEdit(): string {
    switch (this.auctionType) {
      case AuctionType.CMVC:
        return 'FLEX_USER_AUCTIONS_CMVC_OFFER_EDIT';
      case AuctionType.DAY_AHEAD:
        return 'FLEX_USER_AUCTIONS_DAY_AHEAD_OFFER_EDIT';
    }
  }
  ...
  get authoritiesView(): string {
    switch (this.auctionType) {
      case AuctionType.CMVC:
        return 'FLEX_USER_AUCTIONS_CMVC_OFFER_VIEW';
      case AuctionType.DAY_AHEAD:
        return 'FLEX_USER_AUCTIONS_DAY_AHEAD_OFFER_VIEW';
    }
  }
  ...
  ngOnChanges(): void {
    this.columns = this.preparedColumns(COLUMNS, 'auctions.offers').filter(({ field }) => {
      if (this.auctionType === AuctionType.CMVC) {
        return !['schedulingUnit', 'priceKwh', 'volumeKwh', 'potentialFromSU'].includes(field);
      }
      ...
      switch (this.auctionDayAheadType) {
        case 'CAPACITY':
          fields = [...fields, 'priceKwh', 'volumeKwh'];
          break;
        case 'ENERGY':
          fields = [...fields, 'price', 'volume'];
          break;
      }
