...
import { Observable, Subject, debounceTime, distinctUntilChanged, lastValueFrom, startWith, takeUntil, catchError } from 'rxjs';
...
export class AuctionBidComponent implements OnInit, OnDestroy {
  ...
  get isOfferPendingOrNotEdit(): boolean {
    return this.bidData?.status === AuctionOfferStatus.PENDING || !this.isEdit;
  }
    ...
    if (this.auctionId) {
      if (this.auctionType === AuctionType.DAY_AHEAD) {
        this.dayAheadService.getAuction(this.auctionId).subscribe((model: AuctionDayAheadDTO) => {
          ...
          this.isClosedAuction = auctionIsClosed;
        });
      } else {
  ...
  async derChange(index: number, imported = false): Promise<void> {
    ...
    if (!this.isEmptySelfSchedule[index] && !imported) {
      bandDataHours.bandArrays.forEach(dataArray => {
        dataArray.controls.forEach(dataGroup => {
          if (dataGroup.value.bandNumber !== 0) {
            dataGroup.get('price')?.patchValue(null);
          }

          dataGroup.get('volume')?.patchValue(null);
  ...
  save(): void {
    ...
    formData.ders = formData.ders?.map((derItem: { bandData: IDerBandDTO[] | AuctionOfferDerBandDataDTO[] }) => {
      derItem.bandData = (derItem.bandData as IDerBandDTO[])
        ?.map(band => band.data)
        ?.flat()
        .filter(bandData => (!isNil(bandData.price) && !isNil(bandData.volume)) || (!isNil(bandData.volume) && bandData.bandNumber === 0));
      return derItem;
    });
  ...
  private getPotentials(init: boolean = false): void {
    if (this.auctionType === AuctionType.CMVC) {
      this.service.getFlexPotentials(this.cmvcAuction?.id).subscribe(response => {
        if (this.bidData?.flexPotential.id && response.findIndex(item => item.id === this.bidData?.flexPotential.id) <= -1) {
          response.push({
      ...
      this.service.getSchedulingUnits(this.productId).subscribe(response => {
        this.dictionaries.potentials = response;
