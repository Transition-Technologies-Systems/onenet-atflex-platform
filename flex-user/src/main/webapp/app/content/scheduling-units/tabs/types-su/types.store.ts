...
@Injectable()
export class SchedulingUnitsTypesStore extends ComponentStore<DefaultState<SchedulingUnitTypeDTO>> {
  constructor(private service: SchedulingUnitsTypesService) {
  ...
  readonly setData = this.updater((state, data: Pageable<SchedulingUnitTypeDTO>) => ({
    ...state,
    data: data.content,
    totalElements: data.totalElements,
  }));

  readonly data$: Observable<SchedulingUnitTypeDTO[]> = this.select(state => state.data);
