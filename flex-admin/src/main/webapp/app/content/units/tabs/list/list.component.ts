...
import { DirectionOfDeviationType, Role } from '@app/shared/enums';
...
export class UnitsListComponent extends TableExtends implements OnInit, AfterViewInit {
  ...
  showProposal(row: UnitDTO): boolean {
    if (row.fsp.role !== Role.ROLE_FLEX_SERVICE_PROVIDER_AGGREGATED) {
      return !row.schedulingUnit && row.certified && row.balancedByFlexPotentialProduct;
    }

    return !row.schedulingUnit && row.certified && !!row.subportfolio;
  }
