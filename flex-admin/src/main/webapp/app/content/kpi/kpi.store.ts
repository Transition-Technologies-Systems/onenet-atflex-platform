...
export interface KpiGenerateDto {
  type: string;
  dateFrom?: string;
  dateTo?: string;
}
...
@Injectable()
export class KpiStore extends ComponentStore<DefaultState<KpiDTO>> {
  ...
  readonly setData = this.updater((state, data: Pageable<KpiDTO>) => ({
    ...state,
    data: data.content,
    totalElements: data.totalElements,
  }));

  readonly data$: Observable<KpiDTO[]> = this.select(state => state.data);
