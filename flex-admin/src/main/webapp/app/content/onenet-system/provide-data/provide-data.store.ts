...
@Injectable()
export class ProvideDataStore extends ComponentStore<DefaultState<ProvideDataDTO>> {
  ...
  readonly setData = this.updater((state, data: Pageable<ProvideDataDTO>) => ({
    ...state,
    data: data.content,
    totalElements: data.totalElements,
  }));

  readonly data$: Observable<ProvideDataDTO[]> = this.select(state => state.data);
