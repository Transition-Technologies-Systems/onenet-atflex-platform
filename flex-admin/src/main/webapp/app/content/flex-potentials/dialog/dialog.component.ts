...
import { Role, VolumeUnit } from '@app/shared/enums';
...
export class FlexPotentialsDialogComponent extends DialogExtends implements OnInit {
  ...
  get fspRole(): Role {
    const fsp = this.form?.get('fsp')?.value as Dictionary<number>;

    return fsp?.role as Role;
  }

  get selectedProduct(): (Dictionary & { minBidSize: number; maxBidSize: number }) | undefined {
    return this.dictionaries.products.find(({ value }) => value === this.form?.get('productId')?.value);
  }
  ...
  ngOnInit(): void {
    ...
    if (this.mode === 'edit') {
      this.flexPotentialsService.getFlexPotential(this.config.data.id).subscribe((response: FlexPotentialDTO) => {
        ...
        this.getUnits(this.data.fsp?.id);
      });
    } else {
  ...
  save(): void {
    ...
    if (this.mode === 'add') {
      ...
      method = this.flexPotentialsService.update(data, this.selectedFiles);
    }
  ...
  private getUnits(fspId: number | undefined): void {
    ...
    this.flexPotentialsService
      .getUnits(fspId, this.fspRole === Role.ROLE_FLEX_SERVICE_PROVIDER_AGGREGATED, this.data.id)
      .subscribe(response => {
  ...
  private subscribeFspId(): void {
    ...
    this.form
      ?.get('fsp')
      ?.valueChanges.pipe(takeUntil(this.destroy$))
      .subscribe((fsp: Dictionary<number>) => {
        const fspId = fsp.value;

        if (!fspId) {
          this.dictionaries.units = [];
          this.form?.get('unitIds')?.disable();
          return;
        }
