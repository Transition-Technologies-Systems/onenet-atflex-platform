...
export class SchedulingUnitsService extends HttpService {
  protected url = 'flex-server/api/admin/scheduling-units';
  ...
  getSchedulungUnitTypes(): Observable<Dictionary[]> {
    return this.get<Array<Dictionary & { products: ProductDTO[] }>>(`flex-server/api/su-types/minimal`).pipe(
      map(response =>
  ...
  getUnit(id: number): Observable<UnitDTO> {
    return this.get(`flex-server/api/admin/units/${id}`);
  }
  ...
  getLocalizationsDict(unitIds: number[]): Observable<LocalizationTypeDTO[]> {
    ...
    return this.get<LocalizationTypeDTO[]>('flex-server/api/admin/localization-types/get-by-unit-ids', {
      params: {
        unitIds,
      },
    }).pipe(
