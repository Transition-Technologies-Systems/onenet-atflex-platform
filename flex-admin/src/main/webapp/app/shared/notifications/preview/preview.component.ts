...
export class NotificationsPreviewComponent implements OnInit {
    ...
    return Object.entries(this.notificationModel.params).reduce(
      (params: object, [key, data]: [string, { value: string; object: string | null }]) => {
        ...
        if (value && new RegExp(DATE_REGEX).test(value)) {
          ...
          translatedValue = this.translate.instant(`NullValues.NULL_VALUE`);
  ...
  getNotification(key?: string): string {
    const someImportedBids = Object.keys(this.params).includes('IMPORTED_BIDS');
    ...
    if (start === -1) {
      return (
        ...
        (!someImportedBids &&
        !key &&
        this.notificationModel.eventType !== NotificationEvent.DISAGGREGATION_COMPLETED &&
        this.notificationModel.eventType !== NotificationEvent.CONNECTION_TO_ALGORITHM_SERVICE_LOST &&
        this.notificationModel.eventType !== NotificationEvent.DISAGGREGATION_FAILED &&
        this.notificationModel.eventType !== NotificationEvent.SCHEDULING_UNIT_READY_FOR_TESTS
          ? this.translate.instant('labels.none')
          : '')
      );
    }
